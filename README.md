...
5. 기본값으로 현재가 - 2,000원에 1주 매수 주문을 제출합니다.
6. 기존 보유수량이 있으면 현재가 + 2,000원에 매도 주문을 제출합니다. 보유수량이 없다면 매수 체결 후에만 매도를 제출합니다.
7. 주문 후 체결내역과 잔고를 함께 확인해 JSONL 기록을 남깁니다.
8. 기본 주문 한도는 하루 1쌍이며, 미체결 주문이 있으면 중복 주문을 만들지 않습니다.

PDF의 상위 거래 로직은 ±2,000원, 세부 기능 목록은 ±1,000원으로 서로 달라 기본값을 ±2,000원으로 채택했습니다. `.env`의 `PRICE_OFFSET_KRW`로 바꿀 수 있습니다.
